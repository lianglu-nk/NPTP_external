...
for j in range(len(idxSamplesEval)):
    ...
    if not useDenseOutputNetPred:
        if compPerformanceTrain:
            ...
            predSparseTrain[min(idxSamplesEval[j]) + nonzy[0], nonzy[1]] = session.run(predNetworkSparse,
...
idxSamplesEval = [arr[1] for arr in sklearn.model_selection.KFold(n_splits=int(math.ceil(len(testSamples) / batchSize)),
