...
with tf.variable_scope('layer_' + str(layernr)):
    ...
    if nrDenseFeatures > 0.5:
        WlDense = tf.get_variable("W" + str(layernr) + "_dense", trainable=True,
                                  initializer=tf.random_uniform([nrDenseFeatures, layerSizes[layernr]],
                                                                -np.sqrt(initScale / float(layerSizes[layernr - 1])),
                                                                np.sqrt(initScale / float(layerSizes[layernr - 1]))))
        wList.append(WlDense)
    if nrSparseFeatures > 0.5:
        WlSparse = tf.get_variable("W" + str(layernr) + "_sparse", trainable=True,
                                   initializer=tf.random_uniform([nrSparseFeatures, layerSizes[layernr]],
                                                                 -np.sqrt(initScale / float(layerSizes[layernr - 1])),
                                                                 np.sqrt(initScale / float(layerSizes[layernr - 1]))))
    ...
    hiddenActModl = hiddenActl * tf.to_float(
        tf.random_uniform([sh0, tf.shape(hiddenActl)[1]]) < (1.0 - hiddenDropout))
...
for layernr in range(2, len(layerSizes) - 1):
    with tf.variable_scope('layer_' + str(layernr)):
        Wl = tf.get_variable("W" + str(layernr), trainable=True,
                             initializer=tf.random_uniform([layerSizes[layernr - 1], layerSizes[layernr]],
                                                           -np.sqrt(initScale / float(layerSizes[layernr - 1])),
                                                           np.sqrt(initScale / float(layerSizes[layernr - 1]))))
        bl = tf.get_variable('b' + str(layernr), shape=[layerSizes[layernr]], trainable=True,
                             initializer=tf.zeros_initializer())
...
scaleTrain = [tf.assign(idTensors[i], idTensors[i] / (1.0 - inputDropout)).op for i in range(0, len(idTensors))] + [
    tf.assign(hdTensors[i], hdTensors[i] / (1.0 - hiddenDropout)).op for i in range(0, len(hdTensors))]
scalePredict = [tf.assign(idTensors[i], idTensors[i] * (1.0 - inputDropout)).op for i in range(0, len(idTensors))] + [
    tf.assign(hdTensors[i], hdTensors[i] * (1.0 - hiddenDropout)).op for i in range(0, len(hdTensors))]
